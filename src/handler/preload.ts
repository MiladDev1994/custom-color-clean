...
contextBridge.exposeInMainWorld("api_electron", {
    ...
    deleteIdealPoints: (id: any) => ipcRenderer.send('deleteIdealPoints', id),
    ...
    readPrograms: () => {ipcRenderer.send("readPrograms")},
    getAppsList: () => {ipcRenderer.send("getAppsList")},
    deleteApp: (appName: string) => {ipcRenderer.send("deleteApp", appName)},
    addFilterPathToProgram: (values: any) => {ipcRenderer.send("addFilterPathToProgram", values)},
